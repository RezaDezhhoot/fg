...
class HomeComponent extends BaseComponent
{
    ...
    public function render()
    {
        ...
        $this->category_banner = Setting::where('name', 'category_banner')->get()->pluck('value', 'id', 'title', 'url', 'image');
        ...
        $this->categories = Setting::where('name', 'categories')->get()->pluck('value', 'id');
    ...
    public function saveInDatabase()
    {
        ...
        Setting::updateOrCreate(['name' => 'categories'], ['value' => $this->categories]);
    }
    ...
    public function addCategoryBanner($name)
    {
        $this->setting_name = $name;
        $this->resetErrorBag();
        $this->reset(['url', 'title', 'image']);
        $this->emit('showModel', ['CategoryBanner']);
    }
    ...
    public function storeProductUrl()
    {
        ...
        } elseif ($this->setting_name == 'categories') {
            $this->validate([
                'url' => ['required', 'exists:categories,slug'],
            ]);
        } else {
            $this->validate([
    ...
    public function storeCategoryBanner()
    {
        $this->validate([
            'url' => ['required', 'url'],
        ...
        Setting::create([
            'name' => $this->setting_name,
            'value' => json_encode(['url' => $this->url, 'image' => $this->image, 'title' => $this->title])
        ]);

        $this->emit('hideModel', ['CategoryBanner']);
        $this->emitNotify('تنظیمات با موفقیت بروزرسانی شد');
    }
