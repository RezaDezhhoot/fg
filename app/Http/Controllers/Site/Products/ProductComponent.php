...
class ProductComponent extends Component
{
    ...
    public function mount($slug)
    {
        ...
        $this->product = Product::query()->with('articles')->where('slug', $slug)->fgtal()->where('status', '!=', Product::STATUS_DRAFT)->firstOrFail();
		...
		SEOMeta::addMeta('availability', \App\Models\Product::getProductsStatus()[$this->product->status], 'name');
    ...
    private function calculatePrice()
    {
        ...
        $formPrice = $form->reduce(function ($total, $item) {
            ...
            if (FormBuilder::isVisible($this->form, $item) && $formItem['type'] == 'range' && $selectedValue >= ($formItem['options'][0]['value'] ?? 0) && $selectedValue <= (end($formItem['options'])['value'] ?? 1) )  {
