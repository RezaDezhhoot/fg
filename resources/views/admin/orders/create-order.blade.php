<div>
    ...
    <div class="content d-flex flex-column-fluid">
        <div class="container">
            ...
            <x-admin.forms.form title="سفارش" :mode="$mode">
                @if ($details->count() > 0)
                    <x-admin.forms.input type="text" required="true" id="user_number" disabled label="شماره کاربر "
                        wire:model="user_number" />
                @else
                    <x-admin.forms.input type="text" required="true" id="user_number" label="شماره کاربر "
                        wire:model="user_number" />
                @endif
                <x-admin.forms.header title="سبد خرید" />
                <x-admin.button class="btn btn-light-primary font-weight-bolder btn-sm" content=" افزودن محصول  "
                    wire:click="addDetails()" />
                        ...
                        @foreach ($details as $key => $value)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <x-admin.delete-table wire:click="deleteDetail({{ $key }})" />
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
    ...
    <x-admin.modal id="details" size="modal-xl" title="محصول برای سبد خرید">
        <div>
            <x-admin.forms.validation-errors />
        </div>
        <x-admin.forms.select2 id="product_id" required="true" :data="$data['product']" label="محصول "
        ...
        <x-admin.forms.input type="number" id="quantity" label=" تعداد" wire:model="quantity" />
        ...
        <x-admin.forms.input type="number" id="final_total" label="هزینه نهایی (تومان)" disabled
            wire:model.defer="final_total" />
        <x-slot name="footer">
            <button type="button" class="btn btn-primary" wire:click="storeDetails()">ثبت</button>
