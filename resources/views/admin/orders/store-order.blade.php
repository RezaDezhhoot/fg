<div wire:poll.40s = "updateTime()" class="overflow-hidden">
    <x-admin.subheader title="سفارش" :mode="$mode" :create="false" :factor="true" :posttag="true" />
    <div class="row">
            ...
            <div class="content d-flex flex-column-fluid">
                <div class="">
                    ...
                    @if ($allow == true)
                        ...
                        @endpush
                    @endif
                    <x-admin.forms.form title="سفارش (<span class='copy_text'>{{ $model->order->tracking_code }}</span>)"
                        :mode="$mode">
                        @if (auth()->user()->hasPermissionTo('show_order_details'))
                            <div class="form-group col-12">
                                <div class="row">
                                    ...
                                    <table class="table table-striped table-bordered">
                                        ...
                                        <tbody>
                                            <tr>
                                                <td><a
                                                        href="{{ route('admin.users.store', ['action' => 'edit', 'id' => $model->order->user_id]) }}">مشاهده</a>
                                                </td>
                                                <td><a
                                                        href="{{ route('admin.orders', ['filterUserMobile' => $model->order->mobile]) }}">مشاهده</a>
                                                </td>
                                    ...
                                    @if ($model->file)
                                        <div class="col-12">
                                            <img src="{{ asset('media/' . $model->file) }}"
                                        ...
                                        <table class="table table-striped table-bordered">
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td>{{ number_format($model->order->details()->count()) }} عدد
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                        ...
                                        <table class="table table-striped table-bordered">
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td>{{ $payment ? $payment->payment_gateway . ' (' . jalaliDate($payment->updated_at) . ')' : '' }}
                                                    </td>
                                        ...
                                        <table class="table table-striped table-bordered">
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td>{{ number_format(($model->order->discount ?? 0) + ($model->order->voucher_amount ?? 0)) }}
                                                        تومان </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                ...
                                <button class="btn btn-danger" type="button"
                                    onclick="deleteItem({{ $model->id }})"><i class="fa fa-trash"></i> حدف
                                    سفارش</button>
                            </div>
                        @endif
                                        ...
                                        @if (($form['type'] ?? '') != 'paragraph')
                                            ...
                                            <div class="form-group col-2">
                                                @if (isset($form['inserted_price']))
                                                    <td>{{ number_format(($form['inserted_price'] ?? 0) * $model->quantity) }}
                                                        تومان</td>
                                                @else
                                                    <td>{{ number_format(($form['price'] ?? 0) * $model->quantity) }}
                                                        تومان</td>
                                                @endif
                            ...
                            <div class="row" wire:ignore.self>
                                @if ($model->product->type == \App\Models\Product::TYPE_INSTANT_DELIVERY)
                                    ...
                                    @foreach ($model->licenses as $license)
                                        <div oncopy="copy('{{ $license }}')" class="form-group col-12"
                                            wire:ignore.self>
                                            @if ($hash === true)
                                    ...
                                    <div class="form-group col-12" wire:ignore.self>
                                        <button class="btn btn-primary" wire:loading.attr="disabled"
                                            onclick="getNumber()">ارسال مجدد لایسنس</button>
                                    </div>
                                @endif
                            </div>
                        </div>
                        @if (auth()->user()->hasPermissionTo('edit_orders'))
                            ...
                            <x-admin.forms.dropdown id="type" label="وضعیت سفارش" :options="[
                                ...
                                wire:model="orderStatus" />





                            <x-admin.forms.textarea id="note" label="متن پیامک" rows="5"
                                wire:model.defer="orderSms" />
                        @endif
                                    ...
                                    <table class="table  table-bordered">
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>
                                                    <x-admin.edit-table
                                                        href="{{ route('admin.orders.store', ['action' => 'edit', 'id' => $detail->id]) }}" />
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
            ...
            <div class="col-12 col-xl-4">
                <div class="content d-flex flex-column-fluid">
                    <x-admin.forms.form title="">
                                        ...
                                        <tr>
                                            ...
                                            <td class="py-2">
                                                ...
                                                <div class="d-flex justify-content-between my-2">
                                                    @if (!$item->trashed())
                                                        <button wire:click="deleteNote({{ $key }})"
                                                            class="btn btn-light-danger font-weight-bolder btn-sm">حذف
                                                            یادداشت</button>
                                                    @endif
                                                    @if ($item->trashed())
                                                        <span class="label label-lg label-light-danger label-inline">حذف
                                                            شده</span>
                                                    @else
                                                        <P>
                                                            {!! $item->is_user_note
                        ...
                        <x-admin.forms.textarea id="note" label="یادداشت" rows="5"
                            wire:model.defer="sendNote" />
                        <x-admin.forms.dropdown id="note_type" label="نوع یادداشت" :options="['0' => 'خصوصی', '1' => 'عمومی']"
                            wire:model.defer="noteType" />
                        <div class="form-group col-12">
                        ...
                        <x-admin.forms.textarea id="sms" label="ارسال پیامک جداگانه" rows="5"
                            wire:model.defer="sendSms" />
                        <div class="form-group col-12">
