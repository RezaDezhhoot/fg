        ...
        <div id="kt_aside_menu" class="aside-menu my-4" data-menu-vertical="1" data-menu-scroll="1"
            ...
            <ul class="menu-nav">
                @if (request()->routeIs([
                        ...
                        'partner.new.orders',
                        'partner.order',
                        'partner.store.new.orders',
                        'partner.dashboard',
                    ]))
                    <x-admin.sidebar-link href="{{ route('partner.dashboard') }}" :active="request()->routeIs('partner.dashboard')"
                        icons="flaticon-home-2">داشبورد</x-admin.sidebar-link>

                    <x-admin.sidebar-link href="{{ route('partner.new.orders') }}" :active="request()->routeIs(['partner.new.orders', 'partner.store.new.orders'])"
                        icons="fa fa-plus">سفارش جدید
                    </x-admin.sidebar-link>
                    <x-admin.sidebar-link href="{{ route('partner.orders') }}" :active="request()->routeIs(['partner.orders', 'partner.order'])"
                        icons="flaticon2-box-1">سفارش های من
                    </x-admin.sidebar-link>
                    <x-admin.sidebar-link href="{{ route('partner.profile') }}" :active="request()->routeIs('partner.profile')"
                        icons="flaticon-settings-1">پروفایل
                    </x-admin.sidebar-link>
                    <x-admin.sidebar-link href="{{ route('partner.wallet') }}" :active="request()->routeIs('partner.wallet')"
                        icons="flaticon-coins">کیف پول
                    </x-admin.sidebar-link>
                    <x-admin.sidebar-link href="{{ route('partner.setting') }}" :active="request()->routeIs('partner.setting')"
                        icons="flaticon-settings-1">تنظیمات
                    </x-admin.sidebar-link>
                    <x-admin.sidebar-link href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard')" icons="flaticon2-user-1">پنل
                        کاربری
                    </x-admin.sidebar-link>
                @else
                    <x-admin.sidebar-link href="{{ route('admin.dashboard') }}" :active="request()->routeIs('admin.dashboard')"
                        icons="flaticon-home-2">{{ auth()->user()->translate('داشبورد') }}</x-admin.sidebar-link>
                    <li class="menu-section">
                        <h4 class="menu-text">بخش شخصی</h4>
                        <i class="menu-icon ki ki-bold-more-hor icon-md"></i>
                    </li>
                    <x-admin.sidebar-link href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard')"
                        icons="flaticon-user">پروفایل</x-admin.sidebar-link>
                    <x-admin.sidebar-link href="{{ route('admin.my.forms') }}" :active="request()->routeIs('admin.my.forms')"
                        icons="flaticon-user">گزارش های من
                        ({{ auth()->user()->forms()->where('status', \App\Models\AdminForm::PENDING)->count() }})</x-admin.sidebar-link>
                    <li class="menu-section">
                        <h4 class="menu-text">بخش عمومی</h4>
                        <i class="menu-icon ki ki-bold-more-hor icon-md"></i>
                    </li>
                    @can('product_manager')
                        <x-admin.sidebar-link href="{{ route('admin.products.order') }}" :active="request()->routeIs('admin.products.order')"
                            icons="flaticon2-box-1">{{ auth()->user()->translate('سفارشات شما') }}
                        </x-admin.sidebar-link>
                    @endcan
                    ...
                    @can('show_orders')
                        ...
                        <x-admin.multiplesidebar-link title="{{ auth()->user()->translate('سفارشات') }}" :active="request()->routeIs(['admin.orders'])"
                            icons="flaticon2-box-1">
                            <x-admin.sidebar-link href="{{ route('admin.orders') }}" icon="menu-bullet menu-bullet-dot"
                            ...
                            @foreach (\App\Models\Category::whereNull('parent_id')->get() as $item)
                                <x-admin.sidebar-link href="{{ route('admin.orders', ['category' => $item->id]) }}"
                                    icon="menu-bullet menu-bullet-dot" :active="request()->routeIs('admin.orders')"
                                    icons="flaticon2-box-1">{{ auth()->user()->translate($item->title) }}
                    ...
                    @endcan
                    @can('show_comments')
                        <x-admin.sidebar-link href="{{ route('admin.comments') }}" :active="request()->routeIs('admin.comments')"
                            icons="flaticon-comment">نظرات
                            ({{ \App\Models\Comment::where('is_confirmed', 0)->count() }})
                        </x-admin.sidebar-link>
                    @endcan
                    @can('show_questions')
                        <x-admin.sidebar-link href="{{ route('admin.questions') }}" :active="request()->routeIs('admin.questions')"
                            icons="flaticon-chat">پرسش و پاسخ
...
{{--                    @endcan--}}
                    ...
                    @can('show_articles')
                        <x-admin.multiplesidebar-link title="مقالات" :active="request()->routeIs(['admin.articles', 'admin.articlesCat'])" icons="flaticon-notepad">
                            ...
                            <x-admin.sidebar-link href="{{ route('admin.articlesCat') }}"
                                icon="menu-bullet menu-bullet-dot" :active="request()->routeIs('admin.articlesCat')" icons="flaticon-notepad">دسته بندی
                            </x-admin.sidebar-link>
                    ...
                    @endcan
                    @can('show_currencies')
                        <x-admin.sidebar-link href="{{ route('admin.currencies') }}" :active="request()->routeIs('admin.currencies')"
                            icons="flaticon-coins">{{ auth()->user()->translate('واحد پول') }}</x-admin.sidebar-link>
                        <x-admin.sidebar-link href="{{ route('admin.wallet') }}" :active="request()->routeIs('admin.wallet')"
                            icons="flaticon-coins">کیف پول</x-admin.sidebar-link>
                    @endcan
                    @can('show_vouchers')
                        <x-admin.sidebar-link href="{{ route('admin.vouchers') }}" :active="request()->routeIs('admin.vouchers')"
                            icons="flaticon-price-tag">کدهای تخفیف</x-admin.sidebar-link>
                    @endcan
                    ...
                    @can('show_users')
                        ...
                        <x-admin.multiplesidebar-link
                            title="کاربران ({{ App\Models\User::where('auth_status', App\Models\User::UESR_PENDING)->count() }})"
                            :active="request()->routeIs(['admin.users', 'admin.roles'])" icons="flaticon-user">
                            <x-admin.sidebar-link href="{{ route('admin.users') }}" icon="menu-bullet menu-bullet-dot"
                                :active="request()->routeIs('admin.users')" icons="flaticon-user"> کاربران
                                ({{ App\Models\User::where('auth_status', App\Models\User::UESR_PENDING)->count() }})
                            </x-admin.sidebar-link>
                            ...
                            @if (auth()->user()->hasRole('administrator'))
                                <x-admin.sidebar-link href="{{ route('admin.transaction') }}"
                                    icon="menu-bullet menu-bullet-dot" :active="request()->routeIs('admin.transaction')" icons="flaticon-user">تراکنش ها
                                </x-admin.sidebar-link>
                            @endif
                        </x-admin.multiplesidebar-link>
                    @endcan
                    @can('show_products')
                        <x-admin.sidebar-link href="{{ route('admin.products') }}" :active="request()->routeIs('admin.products')"
                            icons="flaticon2-box-1">{{ auth()->user()->translate('محصولات وب سایت ') }}
                        </x-admin.sidebar-link>
                    @endcan
                    @can('depot')
                        <x-admin.sidebar-link href="{{ route('admin.depot') }}" :active="request()->routeIs('admin.depot')"
                            icons="fas fa-warehouse">انبار
                        </x-admin.sidebar-link>
                    ...
                    @endcan
                    @can('show_lottery')
                        <x-admin.sidebar-link href="{{ route('admin.lottery') }}" :active="request()->routeIs('admin.lottery')"
                            icons="flaticon-doc">پنل قرعه کشی</x-admin.sidebar-link>
                    @endcan
                    @can('show_categories')
                        <x-admin.sidebar-link href="{{ route('admin.categories') }}" :active="request()->routeIs('admin.categories')"
                            icons="flaticon-list-3">{{ auth()->user()->translate('دسته ها') }}</x-admin.sidebar-link>
                    @endcan
                    <li class="menu-section">
                    ...
                    @can('reports', 'report_manager')
                        <x-admin.multiplesidebar-link title="گزارش کار ها" :active="request()->routeIs([
                            'admin.forms',
                            'admin.forms.store',
                            'admin.admin.forms',
                            'admin.admin.forms.store',
                        ])" icons="flaticon-list">
                            @can('reports')
                            ...
                            @endcan
                            <x-admin.sidebar-link href="{{ route('admin.admin.forms') }}"
                                icons="menu-bullet menu-bullet-dot" :active="request()->routeIs('admin.admin.forms')">بایگانی ها
                    ...
                    @endcan
                    @can('license')
                        <x-admin.sidebar-link href="{{ route('admin.license') }}" :active="request()->routeIs('admin.license')"
                            icons="flaticon-layer">{{ auth()->user()->translate('لایسنس ها') }}
                        </x-admin.sidebar-link>
                    @endcan
                    @can('show_logs')
                        <x-admin.sidebar-link href="{{ route('admin.logs') }}" :active="request()->routeIs('admin.logs')"
                            icons="flaticon-doc">لاگ ها</x-admin.sidebar-link>
                    @endcan
                    @can('show_analytics')
                        <x-admin.sidebar-link href="{{ route('admin.analytics.vouchers') }}" :active="request()->routeIs('admin.analytics.vouchers')"
                            icons="flaticon-pie-chart-1">آمار کدهای تخفیف
                        </x-admin.sidebar-link>
                    @endcan
                    @if (auth()->user()->hasRole('مدیر محصول'))
                        <x-admin.sidebar-link href="{{ route('admin.users') }}" :active="request()->routeIs('admin.users')"
                            icons="flaticon-user">{{ auth()->user()->translate('کارمندان') }}</x-admin.sidebar-link>
                    @endif
                    <li class="menu-section">
                        <h4 class="menu-text">بخش نظیمات عمومی</h4>
                        <i class="menu-icon ki ki-bold-more-hor icon-md"></i>
                    </li>
                    <x-admin.sidebar-link href="{{ route('admin.myLanguage') }}" :active="request()->routeIs('admin.myLanguage')"
                        icons="flaticon-map-location"> languages
                    </x-admin.sidebar-link>
                    @can('show_settings')
                        <x-admin.sidebar-link href="{{ route('admin.teams') }}" :active="request()->routeIs('admin.teams')"
                            icons="flaticon-user-ok">تیم ما</x-admin.sidebar-link>
                        ...
                        <x-admin.sidebar-link href="{{ route('admin.notifications') }}"
                            icon="menu-bullet menu-bullet-dot" :active="request()->routeIs('admin.notifications')" icons="flaticon-settings-1">اعلانات
                        </x-admin.sidebar-link>
                    ...
                    @endcan
                    @can('show_settings')
                        <x-admin.multiplesidebar-link title="تنظیمات" :active="request()->routeIs([
                            'admin.subjects',
                            ...
                            <x-admin.sidebar-link href="{{ route('admin.subjects') }}" icon="menu-bullet menu-bullet-dot"
                    ...
                    @endcan
                @endif
                <x-admin.sidebar-link href="{{ route('logout') }}" :active="request()->routeIs('logout')"
                    icons="flaticon-logout">خروج</x-admin.sidebar-link>
            </ul>
        </div>
    </div>
</div>
