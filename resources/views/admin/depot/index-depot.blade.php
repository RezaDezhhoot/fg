			...
			<x-admin.modal id="showDetails" size="modal-xl" title="جزئیات محصول">
			   ...
			   @foreach($enter_price as $key => $price)
					<div class="col-12 border">
						<p class="p-2">{{ $loop->iteration }} # تعداد {{ $price->count }} عدد به قیمت {{ number_format((int)$price->price - (((int)$price->rent)/($price->count != 0 ? $price->count : 1))) }} تومان و کرایه بار {{ number_format((int)$price->rent) }} در تاریخ {{jalaliDate($price->created_at, '%d %B %Y')}} وارد انبار شد .</p>
					</div>
				@endforeach
			   </div>
			   <hr>
								...
								<table class="table table-striped">
									...
									@forelse($all as $item)
										<tr>
											...
											<td>
												<div style="cursor:pointer;display: inline-block;background: #f3f6f9;width: 35px;height: 33px;position: relative;top: 5px;right: -7px;border-radius: 4px;">
