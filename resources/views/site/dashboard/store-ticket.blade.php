<div class="w-full mt-[1rem] lg:mt-[0rem] lg:!w-[34rem]">
    <div>
        <a href="{{ route('dashboard.tickets') }}" class="flex items-center px-[2rem] py-[1rem] bg-white w-max rounded-[1rem] cursor-pointer">
    ...
    <form wire:ignore.self>
        <div class="mt-[1rem] bg-white rounded-[1rem] p-[2rem] box-page-create-ticket {{ $ticketStep ? 'hidden' : '' }}" >
            <div class="flex justify-between items-center border-b border-solid pb-[1rem] mb-[1rem] border-[#BDBDC7]">
                <h3 class="font-semibold">ایجاد تیکت جدید</h3>
            ...
            <div wire:ignore.self>
                ...
                <div class="box-radio-form-product w-full flex my-[1rem]">
                    <input class="!w-[50%] !rounded-[0.5rem]" label="مرحله بعد از ثبت سفارش" type="radio" name="first" wire:model="orderStep" value="{{ \App\Models\Subject::AFTER_ORDER }}">

                    <input class="!w-[50%] !rounded-[0.5rem]" label="مرحله قبل از ثبت سفارش" type="radio" name="first" wire:model="orderStep" value="{{ \App\Models\Subject::BEFORE_ORDER }}">
                </div>

                <div class="flex justify-center">
                    <button wire:click="nextStep('subject')" type="button" class="input-submit-style !rounded-[0.5rem] !w-[15rem]">مرحله بعد</button>
                </div>
            </div>
        </div>

        <div class="mt-[1rem] bg-white rounded-[1rem] p-[2rem] box-page-create-ticket {{ $ticketStep == 'subject' ?  '' : 'hidden'}}" >
            <div class="flex justify-between items-center border-b border-solid pb-[1rem] mb-[1rem] border-[#BDBDC7]">
                <h3 class="font-semibold">ایجاد تیکت جدید</h3>
                ...
                @foreach($subjects as $group)
                    <div class="box-radio-form-product w-full flex my-[0.5rem]" wire:ignore>
                        @foreach($group as $subject)
                            <input class="!w-[50%]  !rounded-[0.5rem]" label="{{ $subject['title'] }}" type="radio" name="first" wire:model.defer="ticketSubject" value="{{ $subject['id'] }}">
                        @endforeach

                    </div>
                @endforeach
                <div class="flex justify-center" >
                    <button wire:click="nextStep('description')" type="button" class="input-submit-style !rounded-[0.5rem] !w-[15rem]">مرحله بعد</button>
                </div>
            </div>
        </div>

        <div class="mt-[1rem] bg-white rounded-[1rem] p-[2rem] box-page-create-ticket {{ $ticketStep == 'description' ?  '' : 'hidden'}}">
            <div class="flex justify-between items-center border-b border-solid pb-[1rem] mb-[1rem] border-[#BDBDC7]">
        ...
        <div class="mt-[1rem] bg-white rounded-[1rem] p-[2rem] box-page-create-ticket {{ $ticketStep == 'form' ?  '' : 'hidden'}}">
            <div class="flex justify-between items-center border-b border-solid pb-[1rem] mb-[1rem] border-[#BDBDC7]">
                <h3 class="font-semibold">ایجاد تیکت جدید</h3>
            ...
            <div>
                <div>
                    <input type="text" wire:model.defer="formSubject" class="input-hover w-full bg-[#F8F9FB] border border-[#BDBDC7] rounded-[0.5rem] p-[1rem]" placeholder="لطفا موضوع خود را وارد نمایید">
                    @error('formSubject')
                ...
                @if($orderStep == \App\Models\Subject::AFTER_ORDER)
                    <div class="my-[0.5rem]">
                        <input type="text" wire:model.defer="orderId" class="input-hover w-full bg-[#F8F9FB] border border-[#BDBDC7] rounded-[0.5rem] p-[1rem]" placeholder="کد سفارش">
                        @error('orderId')
                        <small class="text-danger">
                            {{ $message }}
                        </small>
                    ...
                    <div class="my-[0.5rem]">
                        <input type="text" wire:model.defer="productName" class="input-hover w-full bg-[#F8F9FB] border border-[#BDBDC7] rounded-[0.5rem] p-[1rem]" placeholder="نام محصولی خریداری شده">
                        @error('productName')
                        <small class="text-danger">
                            {{ $message }}
                        </small>
                    ...
                    <div class="my-[0.5rem]">
                        <input type="text" wire:model.defer="cardNumber" class="input-hover w-full bg-[#F8F9FB] border border-[#BDBDC7] rounded-[0.5rem] p-[1rem]" placeholder="شماره کارتی که با ان پرداخت انجام داده اید">
                        @error('cardNumber')
                        <small class="text-danger">
                            {{ $message }}
                        </small>
                ...
                <div class="box-radio-form-product w-full my-[0.5rem] grid gap-1 grid-cols-2 lg:grid-cols-4">
                    <input id="Importance" class="!rounded-[0.5rem]" label="کم" type="radio" name="first" value="{{ \App\Models\Ticket::LOW }}" wire:model.defer="priority">
                    <input class="!rounded-[0.5rem]" label="متسوط" type="radio" name="first" value="{{ \App\Models\Ticket::NORMAL }}" wire:model.defer="priority">
                    <input class="!rounded-[0.5rem]" label="زیاد" type="radio" name="first" value="{{ \App\Models\Ticket::HIGH }}" wire:model.defer="priority">
                    <input class="!rounded-[0.5rem]" label="فوری" type="radio" name="first" value="{{ \App\Models\Ticket::NECESSARY }}" wire:model.defer="priority">
                ...
                @enderror
                <div class="my-[0.5rem]">
                    <textarea class="input-hover w-full bg-[#F8F9FB] border border-[#BDBDC7] rounded-[0.5rem] p-[1rem] h-[15rem]" wire:model.defer="body" placeholder="لطفا متن موضوع  خود را وارد نمایید"></textarea>
                    @error('body')
                    <small class="text-danger">
                        {{ $message }}
                    </small>
