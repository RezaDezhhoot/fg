<div>
    <form class="bg-white rounded-2xl p-4 md:p-6" wire:submit.prevent="store()">
        ...
        @if ($errors->any())
            {!! implode(
                '',
                $errors->all('<p class="bg-pink px-4 py-2 text-center rounded-2xl mt-4 font-medium">:message</p>'),
            ) !!}
        @endif
        ...
        @endif
        <label>
            <textarea class="text-field w-full mt-4 h-auto resize-y" rows="4" placeholder="نظر خود را وارد نمایید"
                wire:model.defer="model.comment"></textarea>
        </label>
