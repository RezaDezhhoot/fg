    ...
    <section id="left-dashboard">
        <div wire:ignore class="box-header-dash-comments box-header-dash-comments-sale-ad flex-box flex-justify-space">
            <div class="d-flex h-full">
                <div wire:click="$set('tab','accounts')"
                    class="item-header-dash-comments {{ $tab == 'accounts' ? 'header-dash-comments-active' : '' }}  flex-box">
                    <span>آگهی های من</span>
                </div>

                <div wire:click="$set('tab','saved')"
                    class="item-header-dash-comments {{ $tab == 'saved' ? 'header-dash-comments-active' : '' }} flex-box">
        ...
        <div class="box-message-dash-comments">
                            ...
                            <tbody>
                                @foreach ($items as $item)
                                    <tr class="box-sale-ad-dashboard">
                                        ...
                                        <td
                                            class="{{ $item->status == \App\Models\Account::PENDING ? 'text-warning' : ($item->status == \App\Models\Account::PUBLISHED ? 'color-green' : 'text-danger') }}">
                                            {{ $item->status_label }}</td>

                                        <td class="d-flex flex-column justify-content-center aling-items-center">
                                            <button class="btn-remove-sale-add-dashboard" onclick="deleteItem({{ $item->id }})">حذف</button>

                                            <a href="{{ route('dashboard.sales-ad.create', ['edit', $item->id]) }}"
                                                class="btn-edit-sale-add-dashboard">ویرایش</a>

                                            @if ($item->status == \App\Models\Account::PUBLISHED)
                                                <a href="{{ route('products-sales-ad.show', [$item->id]) }}"
                                                    class="btn-open-sale-add-dashboard">مشاهده آگهی</a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                                ...
                                <div class="box-sale-ad-mobile-my">
                                    ...
                                    <div class="box-sale-ad-mobile-category">
                                        ...
                                        <div class="item">
                                            ...
                                            <p
                                                class="mr-2 {{ $item->status == \App\Models\Account::PENDING ? 'text-warning' : ($item->status == \App\Models\Account::PUBLISHED ? 'color-success' : 'text-danger') }}">
                                                {{ $item->status_label }}</p>
                                        </div>
                                    </div>
                                    ...
                                    @if ($item->status == \App\Models\Account::PUBLISHED)
                                        <div class="flex-box flex-center mt-4">
                                            <a href="{{ route('products-sales-ad.show', [$item->id]) }}"
                                                class="text-primary">مشاهده آگهی</a>
                                        </div>
                                    @endif
            ...
            <div
                class="item-message-dash-comments box-sale-ad-me-lov-dashboard {{ $tab == 'saved' ? '' : 'hide-item' }}">
                @if (sizeof($items) == 0)
                    <div
                            ...
                            <div class="price-box-sale">
                                ...
                                <button type="button" class="book-mark-sale-ad">
                                    <svg width="25" height="25" viewBox="0 0 25 25"
                                        wire:click="unsave('{{ $item->id }}')" fill="black"
