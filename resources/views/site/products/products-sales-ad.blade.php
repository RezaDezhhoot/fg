            ...
            <div id="box-header-box-hever-shop w-full">
                <ul>
                    <a wire:click="$set('category','')">
                        <li class="item-box-hever-store header-box-hever-shop cursor-pointer">
                            <div class="img">
                    ...
                    @foreach($categories as $item)
                        <a wire:click="$set('category','{{$item->id}}')" >
                            <li class="item-box-hever-store header-box-hever-shop {{ $item->id == $category ? "header-box-hever-shop-activate" : "" }} cursor-pointer">
                                <div class="img">
                ...
                <div class="modal-body text-center">
                    <ul class="box-filter-search-mo">
                        <li wire:click="$set('sort','views')" class="item-filter-search-mo">
                            ...
                            <span class="txt-item-filter-search-mo {{ $sort == 'views' ? 'color-blue' : '' }}">پربازدید ترین</span>
                        </li>
                        ...
                        <li wire:click="$set('sort','amount')" class="item-filter-search-mo">
                            ...
                            @endif
                            <span class="txt-item-filter-search-mo {{ $sort == 'amount' ? 'color-blue' : '' }}">گران ترین </span>
                        </li>
                    </ul>
                </div>
            </div>
                ...
                <div class="modal-body text-center modal-filters">
                    <ul>
                        ...
                        <li class="item-filters-search-mo flex-box flex-justify-space">
                            ...
                            <div class="checkbox-container">
                                <input type="checkbox" id="checkbox2" value="1" class="checkbox" wire:model="fg_depot">
...
{{--                    <button class="btn-delete-filter-search-mo">حذف فیلتر</button>--}}
                ...
                <div class="modal-body modal-filters">
                    <div>
                        ...
                        <div class="range-slider" wire:ignore>
                            <input type="range" id="rangeMin2"  max="{{$max }}" value="{{ $min }}"
                                step="10" wire:model="min">
                            <input type="range" id="rangeMax2"  max="{{$max }}" value="{{$max }}"
                                step="10" wire:model="max">
                            <div class="track"></div>
    ...
    <div id="left-search">
        <div class="item-header-main-search flex-box flex-right">
            ...
            <div>
                <ul class="flex-box">
                    <li wire:click="$set('sort','views')" class="margin-horizontal-1 item-filter-header-search {{ $sort == 'views' ? 'color-blue' : '' }}">پربازدید ترین</li>

                    <li wire:click="$set('sort','latest')" class="margin-horizontal-1 item-filter-header-search {{ $sort == 'latest' ? 'color-blue' : '' }}">جدید ترین</li>

                    <li wire:click="$set('sort','amount')" class="margin-horizontal-1 item-filter-header-search {{ $sort == 'amount' ? 'color-blue' : '' }}">گران ترین</li>
                </ul>
            </div>
        </div>
        ...
        <div class="hide-item-mobile">
                <div class="bg-white w-full p-4 mb-4" {{empty($category) ? "" :'hidden'}}  style="border-radius: 1rem">
                    ...
                    <div id="box-header-box-hever-shop-m">
                        <a  wire:click="$set('category','')"  class="item-box-hever-store header-box-hever-shop d-flex flex-col">
                            <div class="img">
                                <img src="https://farsgamer.com/media/6635e71a0ac06.png" alt="">
                            </div>
                        ...
                        @foreach($categories as $item)
                            <a wire:click="$set('category','{{$item->id}}')" class="item-box-hever-store header-box-hever-shop d-flex flex-col">
                                <div class="img">
                                    <img src="{{ asset($item->icon) }}" alt="{{ $item->title }}">
                                </div>
                ...
                <div {{! empty($category) ? "" :'hidden'}} class="bg-white w-full p-4 mb-4  open-box-category  cursor-pointer" style="border-radius: 1rem">
                    ...
                    <div class="message-category-store-mobile hide-item mt-8" id="box-header-box-hever-shop-m">
                        ...
                        @foreach($categories as $item)
                            <a wire:click="$set('category','{{$item->id}}')" class="item-box-hever-store {{ $item->id == $category ? "header-box-hever-shop-activate" : "" }} header-box-hever-shop d-flex flex-col">
                                <div class="img">
                                    <img src="{{ asset($item->icon) }}" alt="{{ $item->title }}">
                                </div>
